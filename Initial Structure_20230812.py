import sys
sys.path.append("C:/Users/한선진/Desktop/2023_Hynix/Netlist_20230812_함수화_V1.py")
sys.path.append("C:/Users/한선진/Desktop/2023_Hynix/Init_Visualization.py")

# NETLIST_230418_수정.txt
from Netlist_20230812_함수화_V1 import *
...
def solve_placement_opt(timelimit, mipGap, sets, parameters, row_restriction):
    ...
    with gp.Env(empty=True) as env:
        ...
        with gp.Model(env=env) as m:
            ...
            # C91
            # for i in list_II:
            #     m.addConstr(sum(s[i][j] + (1 - tau[i][j]) for j in C1.keys() if ((i, j) in II)) >= 1, "c91-" + str(i))
            ...
            for i in C1.keys():
                for j in C1.keys():
                    if (i != j) & (((i, j) in II) | ((j, i) in II)):
                        m.addConstr(psi[i][j] >= s[i][j])
                        m.addConstr(psi[i][j] >= tau[i][j])
                        m.addConstr(psi[i][j] <= s[i][j] + tau[i][j])
                        
            for i in C1.keys():
                m.addConstr(sum(1 - psi[i][j] for j in C1.keys() if (i != j) and (((i, j) in II) or ((j, i) in II))) >= phi[i])
            
            '''
            for i in C1.keys():
                for j in C1.keys():
                    if (i != j) & (((i, j) in II) | ((j ,i ) in II)):
                        m.addConstr(phi[i] >= 1 - s[i][j], "c91-" + str(i) + str(j))
                            
            for i in C1.keys():
                m.addConstr(sum(psi[i][j] for j in C1.keys() if (i != j) and (((i, j) in II) or ((j, i) in II))) >= phi[i], "c92-" + str(i))
            
            for i in C1.keys():
                for j in C1.keys():
                    if (i != j) & (((i, j) in II) | ((j, i) in II)):
                        m.addConstr(psi[i][j] <= 1 - s[i][j], "c93-" + str(i) + str(j))
                        m.addConstr(psi[i][j] <= 1 - tau[i][j], "c94-" + str(i) + str(j))
                        m.addConstr(psi[i][j] >= 1 - s[i][j] - tau[i][j], "c95-" + str(i) + str(j))
...
if __name__ == "__main__":
    mipGap = 0.1
    timelimit = 30
    file_path = "C:/Users/한선진/Desktop/2023_Hynix/NETLIST_230418_수정.txt"
    save_path = "."
